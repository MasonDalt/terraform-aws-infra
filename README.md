...
- **Отсутствие persistent state.** После успешного `apply` попытка выполнить `terraform destroy` локально показала `No changes. No objects need to be destroyed`, хотя ресурсы реально существовали в LocalStack (подтверждено через AWS CLI). Причина — `apply` выполнялся внутри эфемерного контейнера GitLab Runner, и state-файл, созданный Terraform во время применения, удалялся вместе с контейнером сразу после завершения job: backend был локальным, без persistent-хранилища. Это наглядно показывает, зачем в реальных проектах нужен remote backend (S3 + DynamoDB для локов, или GitLab-managed Terraform state) — без него CI-раннер каждый раз "забывает" о созданной инфраструктуре. Осознанно оставлено как известное ограничение и направление для дальнейшего развития проекта.
